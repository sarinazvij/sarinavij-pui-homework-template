...
function addMilk(){
  ...
  let roastCheck = document.querySelector('input[name="flexRadioDefault2"]:checked');
  ...
  if (iceCheck === null || roastCheck === null){
    alert("Please ensure above steps have been completed."); 
      for(i = 0; i < milkType.options.length; i++){
        milkType.selectedIndex = 0;
      }
  }
...
function addSyrup(){
  ...
  if (iceCheck === null || roastCheck === null || milkText === "Select Milk Type"){
    alert("Please ensure above steps have been completed."); 
    for(i = 0; i < syrupType.options.length; i++){
      syrupType.selectedIndex = 0;
    }
  }
...
function clearAll(){
  ...
  medClear.checked = false;
  darkRoastClear.checked = false;
  let milkType = document.getElementById('milk'); // Inspirtation came from: https://linuxhint.com/check-uncheck-checkbox-using-javascript/#:~:text=uncheck%20the%20checkbox.-,Inside%20the%20body%20of%20the%20function%2C%20get%20the%20reference%20of,checked%E2%80%9D%20property%20%E2%80%9Cfalse%E2%80%9D.
  let syrupType = document.getElementById('syrup');
    for(i = 0; i < milkType.options.length; i++){
      milkType.selectedIndex = 0;
    }
